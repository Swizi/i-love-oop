...
fc /B %OUT% %~dp0\\incorrectArgumentsCountOutputCorrect.txt || goto err
echo Test 1 passed
...
fc /B %OUT% %~dp0\\incorrectCommandOutputCorrect.txt || goto err
echo Test 3 passed

:: Incorrect key value
%PROGRAM% decrypt %~dp0\\incorrectKeyInput.txt %~dp0\\incorrectKeyOutput.txt GO > %OUT% && goto err
fc /B %OUT% %~dp0\\incorrectKeyOutputCorrect.txt || goto err
echo Test 4 passed

:: Empty file
%PROGRAM% crypt %~dp0\\emptyInput.txt %~dp0\\emptyOutput.txt 0 || goto err
fc /B %~dp0\\emptyOutput.txt %~dp0\\emptyOutputCorrect.txt || goto err
echo Test 5 passed

:: Standard
%PROGRAM% crypt %~dp0\\standardInput.txt %OUT% 255 || goto err
%PROGRAM% decrypt %OUT% %~dp0\\standardOutput.txt 255 || goto err
fc /B %~dp0\\standardOutput.txt %~dp0\\standardOutputCorrect.txt || goto err
echo Test 6 passed
